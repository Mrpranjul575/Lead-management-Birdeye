...
const LOG_LINES = [
  { text:'Analyzing business data…',          time:'00:02', done:true  },
  { text:'Extracting contact information…',   time:'00:04', done:true  },
  { text:'Calculating AI scores…',            time:'00:06', done:true  },
  { text:'Categorizing intents…',             time:'00:09', done:false },
  { text:'Enriching competitor data…',        time:'00:12', done:false },
  { text:'Generating outreach priorities…',   time:'00:15', done:false },
];

// Fake CSV rows for preview
const PREVIEW_ROWS = [
  { business:'SmileCraft Dental',  email:'sarah@smilecraftdental.com',  city:'Austin TX', industry:'Dental',        intent:'AI Visibility', score:92 },
  { business:'Glow Med Spa',       email:'hello@glowmedspa.com',        city:'Austin TX', industry:'Med Spa',        intent:'Review Growth', score:88 },
  { business:'Prime Wellness',     email:'hello@primewellness.com',     city:'Austin TX', industry:'Wellness',       intent:'AI Visibility', score:76 },
  { business:'Elite Chiropractic', email:'dr@elitechiro.com',           city:'Austin TX', industry:'Chiropractic',   intent:'Listings',      score:65 },
  { business:'Radiant Skin Clinic',email:'ashley@radiantskin.com',      city:'Austin TX', industry:'Dermatology',    intent:'AI Visibility', score:81 },
];
...
export default function BulkCSV() {
  ...
  const T1=dark?'#E6EDF3':'#111827', T2=dark?'#8B949E':'#6B7280';
  const B1=dark?'#30363D':'#E5E7EB', S1=dark?'#161B22':'#FFFFFF';
  const S2=dark?'#0D1117':'#F8F9FA', S3=dark?'#21262D':'#F3F4F5';

  const STAGE_NAMES = ['idle','uploaded','enriching','done'];
  ...
  const handleSaveAll = () => {
    PREVIEW_ROWS.forEach(row => {
      addLead({
        business: row.business, email: row.email, city: row.city,
        industry: row.industry, intent: row.intent, aiScore: row.score,
        contact: row.business, reviews: 0, rating: 0, aiVisibility: 0,
        compGap: 'High', nextAction: 'Initial Outreach',
        lastTouch: 'Never', stage: 'New', cadenceDay: 0, cadenceTotal: 7,
        tags: ['List'], aeNotes: '',
      });
    });
    setSaved(true);
  ...
  const totalRows  = 532;
  const enriched   = Math.floor((progress / 100) * 346);
  const processing = Math.floor((progress / 100) * 186);
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:10, border:`1px solid ${step===3?'rgba(16,185,129,0.3)':B1}`, background: step===3?'rgba(16,185,129,0.06)':S2 }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:12, fontWeight:600, color:T1 }}>{file?.name || 'leads_may30.csv'}</div>
                  <div style={{ fontSize:11, color:T2, marginTop:1 }}>532 rows · {file ? (file.size/1024/1024).toFixed(1) : '2.4'}MB</div>
                </div>
                {step===3
                ...
                {step===3 && (
                  <button onClick={()=>{setStep(0);setFile(null);setLogVisible([]);setProgress(0);}} style={{ padding:4, border:'none', background:'transparent', cursor:'pointer', color:T2, marginLeft:4 }}>
                    <X size={13}/>
                  </button>
                )}
              </div>
              ...
              {step === 2 && (
                <div>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                    <span style={{ fontSize:12, color:T2 }}>AI Enriching Leads…</span>
              ...
              {step === 3 && (
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                  <span style={{ fontSize:12, fontWeight:600, color:'#10B981' }}>✓ Enrichment complete</span>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
                {[
                  ...
                  { label:'Enriched',     value:step===3?346:enriched,  color:'#7C5CE8' },
                  { label:'Processing',   value:step===3?0:processing,  color:'#F59E0B' },
              ...
              <div style={{ background:S2, border:`1px solid ${B1}`, borderRadius:10, padding:'12px 14px' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {logVisible.map((line, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:10, className:'fade-up' }}>
          ...
          <div style={{ background:S1, border:`1px solid ${B1}`, borderRadius:14, overflow:'hidden' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:`1px solid ${B1}` }}>
              <div>
                ...
                <span style={{ fontSize:11, color:T2, marginLeft:8 }}>Showing 5 of 532 leads</span>
              </div>
              <div style={{ display:'flex', gap:8 }}>
                ...
                {!saved && (
                  <button onClick={handleSaveAll} style={{
                    ...
                    <CheckCircle2 size={12}/> Save All 532 Leads
                  </button>
                )}
                {saved && (
            ...
            {showPreview && (
              <div style={{ overflowX:'auto' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1.4fr 90px 100px 110px 60px', gap:0, padding:'8px 20px', background:dark?'rgba(0,0,0,0.2)':'rgba(0,0,0,0.03)', borderBottom:`1px solid ${B1}` }}>
                  {['Business','Email','City','Industry','Intent','Score'].map(h => (
                    <div key={h} style={{ fontSize:10, fontWeight:600, color:T2, textTransform:'uppercase', letterSpacing:'0.05em' }}>{h}</div>
                  ))}
                </div>
                {PREVIEW_ROWS.map((row, i) => (
                  <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1.4fr 90px 100px 110px 60px', gap:0, padding:'10px 20px', borderBottom:`1px solid ${dark?'#21262D':'#F3F4F5'}`, transition:'background 0.1s', cursor:'default' }}
                    onMouseEnter={e=>e.currentTarget.style.background=dark?'rgba(255,255,255,0.02)':'rgba(0,0,0,0.015)'}
                    onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                    <div style={{ fontSize:12, fontWeight:600, color:T1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{row.business}</div>
                    <div style={{ fontSize:11, color:T2, fontFamily:'JetBrains Mono,monospace', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{row.email}</div>
                    <div style={{ fontSize:11, color:T2 }}>{row.city}</div>
                    <div style={{ fontSize:11, color:T2 }}>{row.industry}</div>
                    <div>
                      <span style={{
                        fontSize:10, fontWeight:500, padding:'2px 7px', borderRadius:99,
                        background: row.intent==='AI Visibility'?'rgba(91,63,200,0.15)':'rgba(59,130,246,0.15)',
                        color: row.intent==='AI Visibility'?'#7C5CE8':'#60A5FA',
                      }}>{row.intent}</span>
                    </div>
                    <div style={{ fontSize:12, fontWeight:700, color: row.score>=80?'#10B981':row.score>=65?'#F59E0B':'#EF4444', fontFamily:'JetBrains Mono,monospace' }}>{row.score}</div>
                  </div>
                ))}
              </div>
            )}
